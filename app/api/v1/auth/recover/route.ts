...
export async function POST(request: Request) {
  try {
    ...
    await (await getIdentityService()).requestPasswordReset(body.email, requestSecurityContext(request));
    return Response.json({ data: { accepted: true, message: "If the account is eligible, recovery will be queued." } }, { status: 202 });
  } catch (error) {
